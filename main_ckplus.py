...
from dataset.ckplus_dataset_cv import ImageList
from dataset.sampler import ImbalancedDatasetSampler


#######################################################################################################################################
# Training settings
parser = argparse.ArgumentParser(description='CKPLUS Cross Validation for Facial expression recognition')

# DATA

parser.add_argument('--root_path', type=str, default='../data/CK+/cohn_kanade_images_aligned/',
                    help='path to root path of images')

parser.add_argument('--database', type=str, default='CKPLUS Cross Validation',
                    help='Which Database for train. ()')

parser.add_argument('--train_list', type=str, default = '../data/CK+/metafile/cv/train_ids_0.csv',
                    help='path to training list')

parser.add_argument('--valid_list', type=str, default = '../data/CK+/metafile/cv/test_ids_0.csv',
...
parser.add_argument('--resume', default='checkpoints/affectnet8_model_best.pth.tar', type=str, metavar='PATH',   help='path to latest \
latest checkpoint (default: )')

parser.add_argument('--pretrained', default='checkpoints/affectnet8_model_best.pth.tar', type=str, metavar='PATH', 
...
def main():
    ...
    for fold in range(0,args.folds):
        ...
        val_data = ImageList(root=args.root_path , fileList=args.valid_list,
