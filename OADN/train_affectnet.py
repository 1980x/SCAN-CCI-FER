...
Purpose: FER on Affectnet using OADN
...
from dataset.affectnet_dataset_attentionmaps import ImageList
...
parser.add_argument('--model_dir','-m', default='checkpoints_affectnet8', type=str)

parser.add_argument('--train_list', type=str, default = '../data/Affectnetmetadata/training.csv',                    help='path to training list')
#parser.add_argument('--valid_list', type=str, default =  '../data/Affectnetmetadata/validation.csv',                  help='path to validation list')
#parser.add_argument('--valid_list', type=str, default =  '../data/Affectnetmetadata/occlusion_affectnet_list.txt',        help='path to validation list')
#parser.add_argument('--valid_list', type=str, default =  '../data/Affectnetmetadata/pose_30_affectnet_list.txt',        help='path to validation list')
parser.add_argument('--valid_list', type=str, default =  '../data/Affectnetmetadata/pose_45_affectnet_list.txt',        help='path to validation list')
...
parser.add_argument('--end2end', default=True, elp='if true, using end2end with dream block, else, using naive architecture')
...
def main():
    ...
    print('\n\t\t\t\t Aum Sri Sai Ram\nFER on AffectNet-'+ str(args.num_classes)+' using OADN \n\n')    
    print(args)
    ...
    test_data = ImageList(root='/data3/home/darshan/SAN/validation_affectnet_aligned/', landmarksfile=args.valid_landmarksfile,fileList=args.valid_list,
    ...
    # prepare model
    basemodel = resnet50(pretrained = False, num_classes = args.num_classes)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
           print("Epoch: {}  Best so far Test Acc: {}".format(epoch, prec1))
        ...
        for i, (input, target, attmaps) in enumerate(val_loader):        
            ...
            if i % args.print_freq == 0:
               print('{0} [{1}/{2}]\t'
                  ...
                  'region_loss ({region_loss.avg})\t'
                  'Prec@1  ({top1.avg})\t'
                  .format(mode, i, len(val_loader),  att_loss=att_loss, region_loss=region_loss, top1=top1))
            '''
        print('{0} [{1}/{2}]\t'
                  #'Time {batch_time.val} ({batch_time.avg})\t'
                  'att_loss  ({att_loss.avg})\t'
