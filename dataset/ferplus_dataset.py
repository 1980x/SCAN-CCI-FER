...
def _process_data( emotion_raw, mode): #return emotion based on the mode
        ...
        if mode == 'majority':
            ...
            if sum(emotion) <= 0.5 * sum_list or count > 3: # less than 50% of the votes are integrated, or there are too many emotions, we'd better discard this example
                emotion = emotion_unknown   # force setting as unknown
                #print('Inside',emotion_unknown,sum(emotion))
        #if sum(emotion)==0:
        #   print(emotion_unknown,emotion,sum(emotion))
       ...
       return imgList     
        ...
        with open(fileList) as csvfile:
              ...
              for row in emotion_label:
                  ...
                  #print(image_path,emotion_raw, emotion)
...
if __name__=='__main__':
   #testlist = default_reader('../data/FERPLUS/Dataset/Labels/FER2013Train/label.csv', mode = 'probability')
   #testlist = default_reader('../data/FERPLUS/Dataset/Labels/FER2013Test/label.csv', mode = 'crossentropy')
   #print(testlist[0:2])
   
   imagesize =  224
   transform = transforms.Compose([transforms.Resize((imagesize,imagesize)), transforms.ToTensor()])

   mode = 'probabaility'##'crossentropy'#'majority'
