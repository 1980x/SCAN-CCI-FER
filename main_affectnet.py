'''
Aum Sri Sai Ram
Implementation of               on  Affectnet dataset
...
from dataset.affectnet_dataset import ImageList
...
parser.add_argument('--resume', default='checkpoints/affectnet8_model_best.pth.tar', type=str, metavar='PATH',   help='path to latest checkpoint (default: none)')
...
def main():
    ...
    args = parser.parse_args()
    print('\n\t\t\t\t Aum Sri Sai Ram\nFER on Affectnet using Local and global Attention along with Region branch (non-overlapping patches)\n\n')
    print(args)
    ...
    val_data = ImageList(root=args.root_path, fileList=args.valid_list, num_classes= args.num_classes,
                  transform=valid_transform)   
    
    val_loader = torch.utils.data.DataLoader(val_data, args.batch_size, shuffle=False, num_workers=8)

    train_dataset = ImageList(root=args.root_path, fileList = args.train_list, num_classes= args.num_classes,
    ...
    print('length of AffectNet train Database: ' + str(len(train_dataset)))
    print('length of AffectNet valid Database: ' + str(len(val_loader.dataset)))
