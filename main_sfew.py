...
Title: Implementation of Landmark Guidance Independent Spatio-channel Attention and
Complementary Context Information based Facial Expression Recognition on SFEW dataset
...
from dataset.sfew_dataset import ImageList
...
parser.add_argument('-b_t', '--batch-size_t', default=32, type=int, metavar='N', help='mini-batch size (default: 256)')
...
parser.add_argument('--train_rule', default='Reweight', type=str, help='data sampling strategy for train loader:Resample, DRW,Reweight, None')
...
def main():
    ...
    args = parser.parse_args()
    print('\n\t\t\t\t Aum Sri Sai Ram\nFER on SFEW using Local and global Attention along with region branch (non-overlapping patches)\n\n')
    print(args)
    ...
    val_data = ImageList(root=args.root_path +'Val_Aligned_Faces/', fileList=args.valid_list,
                  transform=valid_transform)   
    
    val_loader = torch.utils.data.DataLoader(val_data, args.batch_size, shuffle=False, num_workers=8)

    train_dataset = ImageList(root=args.root_path+'Train_Aligned_Faces/', fileList = args.train_list,
